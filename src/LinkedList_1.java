...
public class LinkedList_1
{
    ...
    private static void outputLList(LinkedList<LinkedList> list) throws IOException
    {
        ...
        for(int i = 0; i < list.get(i).size(); i++)                 //For however long the list at list i in MasterList
        {
            for(int j = 0; j < 4; j++)                              //For all 4 lists in the MasterList
            {
                temp.add( list.get(j).get(i));                      //Get the ith value from each jth list and add to
            }                                                       //temp
        }
        ...
        for(int i = 0; i < temp.size(); i+=4)                       //For as long as temp was, break into 4s
        {
            ...
            {                                                       //after i
                ...
                {
                    str.append(temp.get(i + n)).append(" ");        //Add the value of temp at that location and a space
                }
